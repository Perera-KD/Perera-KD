<!-- FUTURISTIC HEADER -->

<p align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:0f0c29,50:302b63,100:24243e&text=Perera-KD&fontSize=60&fontColor=00F7FF&animation=twinkling&fontAlignY=38&desc=Full%20Stack%20Developer%20%7C%20Android%20Developer&descAlignY=60&descAlign=50"/>
</p>

<p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Orbitron&size=24&pause=900&color=00F7FF&center=true&vCenter=true&width=600&lines=Full+Stack+Developer;Android+Application+Developer;React+%7C+Firebase+%7C+Java;Designing+Scalable+Applications;Building+Smart+Digital+Systems"/>
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=Perera-KD&color=00f7ff&style=for-the-badge"/>
</p>

---

# 👨‍💻 About Me

```text
> Developer focused on building scalable mobile and web applications
> Interested in system architecture, cloud platforms, and modern frameworks
> Passionate about turning ideas into functional digital systems
```

I design and build **efficient applications** that combine:

⚡ Clean architecture
⚡ Intuitive user interfaces
⚡ Reliable backend systems
⚡ Cloud-powered data infrastructure

My work focuses on **Android development, web platforms, and database-driven applications**.

---

# 🧠 Tech Stack

### Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=java,js,php,sql&theme=dark"/>
</p>

### Frontend

<p align="center">
<img src="https://skillicons.dev/icons?i=react,html,css,tailwind&theme=dark"/>
</p>

### Mobile Development

<p align="center">
<img src="https://skillicons.dev/icons?i=androidstudio&theme=dark"/>
</p>

### Backend & APIs

<p align="center">
<img src="https://skillicons.dev/icons?i=firebase&theme=dark"/>
</p>

### Databases

<p align="center">
<img src="https://skillicons.dev/icons?i=mysql,sqlite&theme=dark"/>
</p>

### Development Tools

<p align="center">
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,idea&theme=dark"/>
</p>

---

# 🚀 Featured Projects

## 📚 Bookstore M-Commerce Application

A **mobile commerce platform for purchasing books** designed for a smooth and efficient digital shopping experience.

### Key Features

⚡ Browse and search books
⚡ Secure checkout process
⚡ Order placement system
⚡ Automatic **PDF invoice generation**
⚡ Invoice download to **internal device storage**
⚡ Push notifications for order updates
⚡ Seamless multitasking within the app

### Tech Stack

```text
Android (Java) • Firebase Database • React Admin Panel • REST APIs
```

---

## 💼 Sales & Accounting System for an Audit Company

A **financial management system built for an audit firm** to streamline sales tracking and accounting processes.

### Key Features

⚡ Sales transaction management
⚡ Financial record tracking
⚡ Bank reconciliation & clearance
⚡ Expense and Asset management
⚡ Internal audit reporting

### Tech Stack

```text
Java • MySQL • AJAX • JavaScript
```


# 🧪 Currently Exploring

```text
Android Architecture Components
Cloud Backend Systems
Scalable Application Design
API Integrations & Automation
```

---

# 🤝 Connect

<p align="center">
Explore my repositories and projects.  
I'm always interested in learning new technologies and improving my development skills.
</p>

<p align="center">
⭐ If you find my work interesting, consider starring the repositories
</p>

---

<p align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&height=140&color=0:24243e,50:302b63,100:00f7ff&section=footer"/>
</p>
